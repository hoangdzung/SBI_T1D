...
from py_replay_bg.sensors import CGM as CGMSensors
...
class T1DModelSingleMeal:
    ...
    def sbi_simulate(self, rbg_data: ReplayBGData, theta: np.ndarray, dss: Optional[DSS], duration: Optional[int] = None) -> List[np.ndarray]:
        ...
        G = np.empty_like(self.G)        
        x = np.zeros_like(self.x)
        CGM = np.empty_like(self.CGM)
    ...
    def simulate(self,
        ...
        # otherwise, set the initial model condition appropriately.
            ...
            if self.previous_day_draws is not None:
        ...
        if is_replay:
            ...
            for k in np.arange(1, self.tsteps):
                ...
                # Hypotreatment generation module
                if dss is not None and dss.enable_hypotreatments:
                    # Call the hypotreatment handler
                    ht, dss = dss.hypotreatments_handler(self.G[0:k],
                                                         meal_announcement[0:k],
                                                         meal_type[0:k],
                ...
                # Correction bolus delivery module if it is enabled
                if dss is not None and  dss.enable_correction_boluses:
                    # Call the correction boluses handler
                    cb, dss = dss.correction_boluses_handler(self.G[0:k],
                                                             meal_announcement[0:k],
                                                             meal_type[0:k],
